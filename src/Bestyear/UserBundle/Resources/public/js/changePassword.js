...
$(document).ready(function () {
    $("#currentPasswordSpan input").focus(); 
});
...
function checkCurrentPass() {
    currentPassword = $("#currentPasswordSpan input");
    error = false;
    message = null;
    
    if (currentPassword.val() && currentPassword.val().length < minSizePassword) {
        error = true;
        message = "6 caractères min.";
    }
    
    if (error) {
        currentPassword.parents('span').addClass('inputError');
        currentPassword.attr("title", message);
    } else {
        removeErrors(currentPassword);
    }
}
 ...
 */
$('#currentPasswordSpan input')
    .keyup(function() {checkCurrentPass();})
    .blur(function() {checkCurrentPass();});
